...
We recommend using a unique session id for each user session. This can be a random string or a unique identifier that you generate for each user.
The session should not be long lived, this is not something like a cookie that you store for a long time. 
It should be generated for each user session and it should change if the user closes the tab/window.
Failing to do so could lead to a lot of falso positives when checking the session data due to pollution from other sessions of the same user.