...
Due to false positives, banning the user is never a good idea. Even if behind a proxy or VPN, a user might be legitimate. Instead of banning we decided to allow our users to enable or disable certain payment gateways.
We have clients that allow users to pay with crypto or other methods that are not affected by the proxy detection.
